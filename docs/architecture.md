...
- **Secure Admin Portal**: `/admin/login` + `/admin/portal` using NextAuth credentials authentication and a persistent sidebar for Overview, Inventory, Orders, Analytics.
- **Interactive 3D Customizer**: `/customizer` uses React Three Fiber + Drei with 360° controls and swatch-based variant color updates for hats/phone cases.
- **Peaches Loyalty Hub**: `/peaches` provides VIP dashboard UX with progress ring, tier unlocks, reward timeline, exclusive drops section, and refer-a-friend copy-link action.
